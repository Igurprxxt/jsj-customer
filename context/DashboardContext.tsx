...
const DashboardProvider = ({ children }: any) => {
  ...
  const { globalQueries, setLoading, loadAllApis } = useQuery();
  ...
  const getDateStats = async (date: any, cb: any) => {
    setDateStatsLoading(true);
    const res: any = await dateStatsService({
      query: {
        ...globalQueries,
        startDate: date,
        endDate: date,
  ...
  // useEffect(() => {
  //   allServices();
  // }, [globalQueries, selectPnl, loadAllApis === 'reload']);
