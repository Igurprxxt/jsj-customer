...
export const SelectPnl = ({
  ...
  const renderItem = ({ item }: { item: any }) => (
    <TouchableOpacity
      ...
      {item?.icon && item?.icon}
      <Text className={cn('font-poppinsMedium', 'text-tertiary dark:text-white')}>
        {item?.label}
      </Text>
    </TouchableOpacity>
  );
  ...
  return (
    <View className={cn('z-[9999] mb-1', containerClassName)}>
      <View className="flex-row items-center justify-between gap-2">
        <TouchableOpacity
          className={cn(
            'w-[85%] flex-row items-center  rounded-md  p-2',
            'border-[1px] border-gray-300 bg-white dark:border-0 dark:bg-analytics_card',
            className,
            buttonClassName
          )}
          onPress={toggleDropdown}>
          <Text className={cn('flex-1', 'text-base text-tertiary dark:text-dashboard_card_text')}>
            {value}
          </Text>
          <Ionicons
