...
const Profile = () => {
  const { user, updateProfile, btnLoading, setUser } = useAuth();
    ...
    updateProfile(
      {
        ...
        body: {
          ...
          ...(userData?.email !== user?.email && { email: userData?.email }),
          ...
          removeImage: profileImage?.removeImage || false,
        },
      },
      () => {
              ...
              <View className="flex-col items-center gap-y-2">
                {profileImage?.uri || user?.profileImage ? (
                  <View className="relative h-[140px] w-[140px] rounded-full ">
                    ...
                    {/* <TouchableOpacity
                      onPress={pickImage}
                      className="absolute -right-1 bottom-7 z-10  ">
                      <Feather name="edit" size={24} className="!text-primary dark:!text-black" />
                    </TouchableOpacity> */}
                  </View>
                ) : (
                  <FontAwesome6
                ...
                <View className="flex-row items-center justify-center gap-x-4">
                  <TouchableOpacity>
                    <Text
                      onPress={pickImage}
                      className="mt-2 cursor-pointer font-interMedium text-primary">
                      {user?.profileImage || profileImage?.uri ? 'Change Photo' : 'Upload Photo'}
                    </Text>
                  </TouchableOpacity>
                  {user?.profileImage || profileImage?.uri ? (
                    <TouchableOpacity>
                      <Text
                        onPress={removeImage}
                        className="mt-2 cursor-pointer font-interMedium text-primary">
