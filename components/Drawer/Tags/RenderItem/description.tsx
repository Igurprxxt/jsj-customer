...
const Description = ({ tag, onEdit }: any) => {
  ...
  return (
    <View
      key={tag.uuid}
      className="flex-1 flex-row  gap-2 rounded-lg   bg-gray-100 px-4 py-2 dark:bg-dashboard_card ">
      <View className="flex-1 ">
        <Text className="font-poppinsMedium text-base text-black dark:text-white">{tag?.name}</Text>
        {tag?.description ? (
          <Text
            numberOfLines={2}
            className="font-poppinsMedium text-xs text-tertiary dark:text-gray-500 ">
            {tag?.description}
          </Text>
        ) : (
          <></>
