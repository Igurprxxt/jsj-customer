...
export default function RenderItem({ item, expandedId, toggleSection, handleSelect }: any) {
  return (
    <View className={cn('px-5 dark:bg-dashboard_card')} key={item.id}>
      <TouchableOpacity
        className="my-2 flex-row items-center "
        onPress={() => toggleSection(item.id)}>
        <View className="flex-1 flex-row items-center gap-5 ">
          <Text className={cn('text-base text-tertiary dark:text-white')}>{item.heading}</Text>
          {item?.selectedValue ? (
            <Text className="font-poppinsMedium text-sm capitalize  text-primary">
              ({item.selectedValue})
            </Text>
          ) : (
            <></>
        ...
        item?.data?.map((subItem: any) => (
          <View key={subItem.id} className="ml-4">
            <TouchableOpacity
              ...
              <Text className={cn('font-poppinsMedium text-sm text-tertiary dark:text-white')}>
                {subItem.label}
              </Text>
